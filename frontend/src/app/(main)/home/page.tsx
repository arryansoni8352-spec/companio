      ...
      <div className="stories-bar">
        ...
        <div className="story-item">
          <div className="story-ring" style={{ background: 'var(--border-medium)', position: 'relative' }}>
            <div style={{ width: '100%', height: '100%', borderRadius: '50%', border: '3px solid var(--bg-primary)', background: 'var(--bg-tertiary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, color: 'var(--color-primary)' }}>+</div>
          </div>
        ...
        {stories.map((group, i) => (
          <div key={i} className="story-item">
            <div className="story-ring">
              <div style={{ width: '100%', height: '100%', borderRadius: '50%', border: '3px solid var(--bg-primary)', background: 'linear-gradient(135deg, var(--color-primary), var(--color-accent))', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: 18 }}>
      ...
      <div style={{ maxWidth: 614, margin: '0 auto' }}>
        {loading ? (
          <div style={{ padding: 'var(--space-4)' }}>
            {[1, 2, 3].map((i) => (
              <div key={i} className="post-card" style={{ marginBottom: 'var(--space-4)' }}>
          ...
          posts.map((post, index) => (
            <div key={post.id} className="post-card">
              <div className="post-header">
                <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'linear-gradient(135deg, var(--color-primary), var(--color-secondary))', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: 16 }}>
              ...
              {post.media && post.media.length > 0 ? (
                <div style={{ width: '100%', aspectRatio: '1', background: 'linear-gradient(135deg, var(--color-primary-alpha), var(--bg-tertiary))', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-tertiary)', fontSize: 64 }}>📷</div>
              ) : (
                <div style={{ padding: 'var(--space-6)', fontSize: 'var(--font-size-lg)', lineHeight: 1.6 }}>{post.content}</div>
              )}
